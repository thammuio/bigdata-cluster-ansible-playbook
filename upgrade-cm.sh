...
if [ $1 == "--list-tags" ] || [ $1 == "--list-tasks" ];then
	if [ $1 == "--list-tags" ];then
		...
		echo "################### List of Available Tags to Run ########################"
		echo "##########################################################################"
		ansible-playbook playbooks/upgrade-cm.yml --list-tags --extra-vars "user=sudouser"
		exit 0
	elif [ $1 == "--list-tasks" ];then
		echo "##########################################################################"
		echo "########## Listing all Tasks for all Plays/all tags     ##################"
		echo "##########################################################################"
		ansible-playbook playbooks/upgrade-cm.yml --extra-vars "user=sudouser" --list-tasks
		exit 0
	fi
elif [[ $1 == "--check" ]]; then
	usage_help
	...
	echo "##########################################################################"
	ansible-playbook playbooks/upgrade-cm.yml --extra-vars "user=sudouser" --tags $1 --list-tasks
elif [[ $2 == "--check" ]]; then
	echo "##########################################################################"
	echo "####################### Dry Run for Given Tags      ######################"
	echo "##########################################################################"
	ansible-playbook playbooks/upgrade-cm.yml --extra-vars "user=sudouser" --tags $1 --check
elif [[ $1 != "--check" ]] && [[ $2 != "--check" ]]; then
	echo "##########################################################################"
	echo "########### Actual Install Run for Given Tags ############################"
	echo "##########################################################################"
	ansible-playbook playbooks/upgrade-cm.yml --extra-vars "user=sudouser" --tags $1
else
	usage_help
fi
